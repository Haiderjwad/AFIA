...
import { DashboardCardProps, MenuItem } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ onProductClick, onNavigate, lowStockItems }) => {
              ...
              {showNotifications && (
                <div className="absolute left-0 mt-4 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-[100] overflow-hidden animate-in zoom-in-95">
                  <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                    ...
                    <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{lowStockItems.length} جديد</span>
                  </div>
                  <div className="max-h-64 overflow-y-auto">
                    {lowStockItems.length === 0 ? (
                      ...
                      lowStockItems.map(item => (
                        <div key={item.id} className="p-3 border-b border-gray-50 hover:bg-red-50 transition-colors flex items-start gap-3">
